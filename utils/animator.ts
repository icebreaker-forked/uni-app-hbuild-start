...
export class Animator<TAnimationValue = number> {
	...
	constructor(options: Partial<AnimationOptions<TAnimationValue>>) {
		...
		if (typeof this.options.from === 'object' && this.options.from !== null) {
			...
			this.velocity = velocityObj as TAnimationValue extends number ? number : Record<string, number>;
		} else {
			...
			onUpdate: (value: TAnimationValue) => {
				...
				if (timeDelta > 0) {
					...
					if (typeof value === 'number' && typeof previousValue === 'number') {
						...
						this.velocity = (rawVelocity / 1000) as TAnimationValue extends number ? number : Record<string, number>; // Normalize to 0-1 range
	...
	private shouldReverse(): boolean {
		if (typeof this.currentValue === 'number' && typeof this.options.from === 'number' && typeof this.options.to === 'number') {
